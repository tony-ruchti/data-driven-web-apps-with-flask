...
# add source /app/venv/bin/activate to .zshrc to load vm when logging in
...
# run pypi.service ExecStart command to fine tune paths make sure uWSGI runs appropriately, from RuntimeDirectory?
